    ...
    <section class="product-thumbnails">
        <div class="container">
            <div class="row no-gutters justify-content-center">
                <div class="col-md-10">
                    <div class="row no-gutters justify-content-center">
                        @foreach ($exVessels as $item)
                            <div class="col-md-6" data-aos="zoom-in">
                                <div class="product-wrapper">
                                    <div class="product-area">
                                            @if($item->is_sold == 1)
                                                <div class="sold">
                                                    <img src="{{ asset('assets/frontend/assets/img/sold3.png') }}" style="" alt="PT. Arjaya Berkah Marine">
                                                </div>
                                            @endif
                                        ...
                                        <div class="image-area shadow">
                                            <div class="image-slider">
                                                @foreach ($item->images as $ijo)
                                                    <div class="slider">
                                                        <div class="image-wrapper selector" data-exthumbimage="{{ url('storage/upload_files/images/exvessel/large') . '/' . $ijo->uri }}" data-src="{{ url('storage/upload_files/images/exvessel/large') . '/' . $ijo->uri }}">
                                                            <img src="{{ url('storage/upload_files/images/exvessel/small') . '/' . $ijo->uri }}" alt="{{ $item->title }}">
                                        ...
                                        <ul class="tag">
                                            <li>
                                                <span>{{ $item->vesselType->svalue }}</span>
                                            </li>
                                        </ul>
                                        <div class="desc-area">
                                            <h5>{{ $item->title }}</h5>
                                        ...
                                        <ul class="button-area flat">
                                            ...
                                            <li>
                                                <button class="button" data-toggle="modal" data-target="#modal-product" data-product="{{ $item->title }}" data-name="{{ $item->title }} }}">Hubungi Kami</button>
                                            </li>
                                        </ul>
                                    </div>
                                </div>
            ...
            @if ($exVessels->hasPages())
                <nav class="pagination-area">
                    <ul class="pagination justify-content-center">
                        {{-- Previous Page Link --}}
                        @if ($exVessels->onFirstPage())
                        ...
                        @else
                            <li class="page-item" data-aos="zoom-in">
                                <a class="page-link" href="{{ $exVessels->previousPageUrl() }}" tabindex="-1" aria-disabled="true"><i class="fas fa-angle-left"></i></a>
                            </li>
                        @endif
                        {{-- Pagination Elements --}}
                        @foreach ($exVessels->links()->elements as $element)
                            {{-- Array Of Links --}}
                            @if (is_array($element))
                                @foreach ($element as $page => $url)
                                    @if ($page == $exVessels->currentPage())
                        ...
                        @endforeach
                        {{-- Next Page Link --}}
                        @if ($exVessels->hasMorePages())
                            <li class="page-item" data-aos="zoom-in">
                                <a class="page-link" href="{{ $exVessels->nextPageUrl() }}"><i class="fas fa-angle-right"></i></a>
                            </li>
                        @else
                            <li class="page-item disabled" data-aos="zoom-in">
