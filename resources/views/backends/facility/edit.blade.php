...
<div class="row">
    <div class="col-12">
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <form action="{{ route('facility.update', $facility->id) }}" method="POST" class="row g-3 needs-validation" novalidate enctype="multipart/form-data">
                    @csrf
                    @method('PATCH')
                    <x-form.input name="title" label="Title name" :value="$facility->title" :required="true" />
                    <x-form.textarea name="description" label="Description" :value="old('description', $facility->description)"  :rich="true" />
                    <x-form.select name="is_active" label="Is Active" :options="[1 => 'Active', 0 => 'In Active']" :selected="$facility->is_active ?? ''" :required="true"/>
                    <div class="col-12">
                        <a href="{{ route('facility') }}" class="btn btn-danger">
