...
<div class="row">
    <div class="col-12">
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <form action="{{ route('faq.update', $faq->id) }}" method="POST" class="row g-3 needs-validation" novalidate enctype="multipart/form-data">
                    @csrf
                    @method('PATCH')
                    <x-form.select name="faq_category_id" label="Kategori Faq" :options="$faqCategories" :selected="$faq->faq_category_id ?? ''" :required="true"/>
                    <x-form.input name="position" label="Position" :value="$faq->position" :required="true" />
                    <x-form.textarea name="question" label="Question" :value="old('question', $faq->question)"  :rich="true" />
                    <x-form.textarea name="answer" label="Answer" :value="old('answer', $faq->answer)"  :rich="true" />
                    <x-form.select name="is_active" label="Is Active" :options="[1 => 'Active', 0 => 'In Active']" :selected="$faq->is_active ?? ''" :required="true"/>
                    <div class="col-12">
