...
<div class="row">
    <div class="col-12">
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <form class="row g-3 needs-validation" method="POST" action="{{ route('exvessel.store') }}" novalidate enctype="multipart/form-data">
                    @csrf
                    ...
                    <x-form.select name="vessel_type" label="Tipe Kapal" :options="$vesselType" :selected="old('vessel_type')" :required="true"/>
                    <x-form.input name="grt" label="GRT" :value="old('grt')" />
                    <x-form.input name="loa" label="LOA" :value="old('loa')" />
                    <x-form.input name="dwt" label="DWT" :value="old('dwt')" />
                    <x-form.input name="year" label="Year" :value="old('year')" />
                    <x-form.input name="engine" label="Engine" :value="old('engine')" />
                    <x-form.select name="is_sold" label="Is Sold" :options="$soldType" :selected="old('is_sold')" :required="true"/>
                    <x-form.select name="is_active" label="Is Active" :options="[1 => 'Active', 0 => 'In Active']" :selected="old('is_active')" :required="true"/>
                    <x-form.file name="image" label="Ex Vessel Picture" />
                    <div class="col-12">
