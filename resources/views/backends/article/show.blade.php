...
<div class="modal fade" id="addImageModal" tabindex="-1" aria-labelledby="addImageModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <form action="{{ route('article.image.store', ['article' => $article->id ]) }}" method="POST" enctype="multipart/form-data">
            @csrf
...
<div class="row">
                ...
                <div class="card-body">
                    @foreach($article->images as $item)
                        ...
                        <div class="modal fade" id="setDefaultModal" tabindex="-1" aria-labelledby="setDefaultModalLabel" aria-hidden="true">
                            <div class="modal-dialog">
                                <form action="{{ route('article.image.set_default', ['article' => $article->id, 'image' => $item->id]) }}" method="POST">
                                    @csrf
                        ...
                        <div class="modal fade" id="deleteImageModal" tabindex="-1" aria-labelledby="deleteImageModalLabel" aria-hidden="true">
                            <div class="modal-dialog">
                                <form action="{{ route('article.image.delete', ['article' => $article->id, 'image' => $item->id]) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="col-8">
            <div class="card mb-4">
                <div class="card-header d-flex justify-content-between align-items-center">
                    {{ $article->title }}
                    <a href="{{ route('article') }}" class="btn btn-primary btn-sm">
