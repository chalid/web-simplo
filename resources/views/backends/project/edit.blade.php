...
<div class="row">
    <div class="col-12">
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <form action="{{ route('project.update', $project->id) }}" method="POST" class="row g-3 needs-validation" novalidate enctype="multipart/form-data">
                    @csrf
                    @method('PATCH')
                    <x-form.input name="title" label="Title name" :value="$project->title" :required="true" />
                    <x-form.textarea name="description" label="Description" :value="old('description', $project->description)"  :rich="true" />
                    <x-form.select name="project_category_id" label="Kategori Project" :options="$projectCategories" :selected="$project->project_category_id ?? ''" :required="true"/>
                    <x-form.select name="is_active" label="Is Active" :options="[1 => 'Active', 0 => 'In Active']" :selected="$project->is_active ?? ''" :required="true"/>
                    <div class="col-12">
                        <a href="{{ route('project') }}" class="btn btn-danger">
